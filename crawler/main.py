...
# 사용 전 세션 체크.
# python3 ./download_profiles.py -l ddunddun333 -f -s -hl -r -sid $session_id
# 인스타 로그인 후 session id 가져와서 입력해줘야함.
# Todo 웹용으로 전환하면서, 성공, 실패 여부 return 하는 것으로. 다운로드 전 id 저장, 다운로드 에러 발생 시 해당 id와 error message return.
...
def main():
    ...
    parser.add_argument("--profiles", help="다운로드할 프로필 목록 파일 (기본값: profiles.txt)")
    ...
    profiles_file = args.profiles or base_dir / "profiles.txt"
    ...
    hasSid = args._get_args().__contains__('sid')

    log(f"getArgs(sid) =  ${hasSid}")
    ...
    if session_id:
        ...
        l.context._session.cookies.set('sessionid', '75098765446%3AlIzFfRW9US2paa%3A10%3AAYgeo_vLuF60CdQPyhHsMmS6MtT9wHFr2I29s6bRLg') # 여기에 session_id를 넣어야함.
