...
const AccountChip = ({
  ...
  <button
    type="button"
    onClick={onClick}
    className={`group flex min-w-[7rem] items-center gap-3 rounded-full border px-4 py-2 text-left transition ${
      isActive
        ? 'border-brand-400 bg-brand-400/10 text-white shadow-lg shadow-brand-400/30'
        : 'border-white/10 bg-white/5 text-slate-200 hover:border-white/20 hover:bg-white/10'
    }`}
  >
    <span className="relative inline-flex h-10 w-10 shrink-0 rounded-full border border-white/10 shadow-md shadow-black/40">
      <img
  ...
  return (
    <div className="flex gap-3 overflow-x-auto pb-1">
      <button
        ...
        className={`flex items-center rounded-full border px-4 py-2 text-sm font-medium transition ${
